...
namespace vaultke\foundation\templates\crud;
...
class Generator extends \yii\gii\Generator
{
    ...
    public function generate()
    {
        ...
        $routePath = Yii::getAlias('@app/app/routes/');
        $routerFile = $routePath.ucwords($this->getControllerID()).'Router.php';
        $files[] = new CodeFile($routerFile, $this->render("router.php"));

        $routePath = Yii::getAlias('@app/devops/OAS/tags/');
        $routerFile = $routePath.ucwords($this->getControllerID()).'Schema.php';
        $files[] = new CodeFile($routerFile, $this->render("tag.php"));

        // $basePath = Yii::getAlias('@app/')
        // //$templatePath = Yii::getAlias('@app/'$this->getTemplatePath() . '/views';
        ...
        foreach ($columns as $column => $type) {
            if($column!=='is_deleted'){
                switch ($type) {
                    ...
                    case Schema::TYPE_DATETIME:
                    case Schema::TYPE_TIMESTAMP:
                        $hashConditions[] = "'{$column}' => \$this->search,";
                        break;
                    default:
                        $likeKeyword = $this->getClassDbDriverName() === 'pgsql' ? 'ilike' : 'like';
                        $likeConditions[] = "->orFilterWhere(['{$likeKeyword}', '{$column}', \$this->search])";                    
                        break;
                }
            }
        }
    ...
    public function generateActionParamComments()
    {
        ...
        return ['@param string  $' . $model->cryptKey . '_crypt_id'];
    }
